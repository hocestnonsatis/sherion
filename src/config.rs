...
impl Config {
    ...
    pub fn foreground_brush(&self) -> AlphaColor<Srgb> {
        parse_color(&self.colors.foreground).unwrap_or_else(|_| {
            AlphaColor::from_rgb8(0xcc, 0xcc, 0xcc)
        })
    }

    pub fn background_brush(&self) -> AlphaColor<Srgb> {
        parse_color(&self.colors.background).unwrap_or_else(|_| {
            AlphaColor::from_rgb8(0x1e, 0x1e, 0x1e)
        })
    }

    pub fn cursor_brush(&self) -> AlphaColor<Srgb> {
        parse_color(&self.colors.cursor).unwrap_or_else(|_| {
            AlphaColor::from_rgb8(0xff, 0xff, 0xff)
        })
    }
